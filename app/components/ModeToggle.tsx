...
const ModeToggle = () => {
    const [isDarkMode, setIsDarkMode] = useState(Cookie.get('darkMode') === "true" ? true : false);
    ...
    useEffect(() => {
        const storedMode = Cookie.get('darkMode');
        setIsDarkMode(storedMode ? JSON.parse(storedMode) : false);
        document.documentElement.setAttribute('data-theme', isDarkMode ? 'business' : 'corporate');
    }, []);

    useEffect(() => {
        Cookie.set('darkMode', JSON.stringify(isDarkMode));
    }, [isDarkMode]);
    ...
    return (
        <div>
            <label className="swap swap-rotate">
                ...
                <input type="checkbox" checked={isDarkMode} onClick={handleToggle} />
